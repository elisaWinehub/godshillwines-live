...
onDocumentLoaded(() => {
  ...
  // ResizeObserver recomputes after load (God's Hill custom header replaces Horizon markup)

  if (headerGroup) {
    const resizeObserver = new ResizeObserver(() => {
      window.requestAnimationFrame(() => updateHeaderHeights());
    });
    ...
    const customShell = headerGroup.querySelector('.shopify-section.gh-custom-store-header .gh-hdr-shell');
    if (!(header instanceof HTMLElement) && customShell instanceof HTMLElement) {
      resizeObserver.observe(customShell);
    }
    ...
    const mutationObserver = new MutationObserver(() => {
      const nextChildren = headerGroup.children;
