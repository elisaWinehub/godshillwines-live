...
/**
 * Resolve Horizon `header-component` and/or God's Hill custom header (.gh-custom-store-header) metrics.
 * @param {HTMLElement | null} [headerGroup]
 */
function resolvePrimaryHeaderParts(headerGroup = document.querySelector('#header-group')) {
  /** @type {HTMLElement | undefined} */
  const horizonHeader = /** @type {HTMLElement | undefined} */ (
    headerGroup?.querySelector('header-component') ?? undefined
  );
  const customHeaderSection = headerGroup?.querySelector('.shopify-section.gh-custom-store-header');
  const customShell = /** @type {HTMLElement | null} */ (
    customHeaderSection?.querySelector('.gh-hdr-shell') ?? null
  );
  const primaryMeasured = /** @type {HTMLElement | null} */ (
    horizonHeader instanceof HTMLElement ? horizonHeader : customShell
  );
  const primarySection = /** @type {HTMLElement | null} */ (
    horizonHeader instanceof HTMLElement ? horizonHeader.closest('.shopify-section') : customHeaderSection
  );
  return {
    horizonHeader:
      horizonHeader instanceof HTMLElement ? /** @type {HTMLElement | null} */ (horizonHeader) : null,
    customShell,
    primaryMeasured,
    primarySection,
  };
...
/**
 * Sum of `#header-group` children excluding the primary header section plus Horizon transparent-overlay rules.
 * @param _legacyHeader Unused; kept for call-site compatibility
 * @param {HTMLElement | null} [headerGroup]
 * @returns {number}
 */
export function calculateHeaderGroupHeight(
  _legacyHeader = undefined,
  headerGroup = document.querySelector('#header-group')
) {
  const { horizonHeader, primaryMeasured, primarySection } = resolvePrimaryHeaderParts(headerGroup);
  if (!headerGroup || !(primaryMeasured instanceof HTMLElement)) return 0;

  let totalHeight = 0;
  for (let i = 0; i < headerGroup.children.length; i++) {
    const element = headerGroup.children[i];
    if (!(element instanceof HTMLElement)) continue;
    if (primarySection instanceof HTMLElement && element === primarySection) continue;
...
export function updateHeaderHeights() {
  const headerGroup = document.querySelector('#header-group');
  const { horizonHeader, customShell, primaryMeasured } = resolvePrimaryHeaderParts(headerGroup);

  if (!(primaryMeasured instanceof HTMLElement)) return;

  const headerHeight = primaryMeasured.offsetHeight;
  const headerGroupHeight = calculateHeaderGroupHeight(undefined, headerGroup);
  const headerTopRow =
    horizonHeader instanceof HTMLElement
      ? /** @type {HTMLElement | null} */ (horizonHeader.querySelector('.header__row--top'))
      : null;
  ...
  if (headerTopRow instanceof HTMLElement) {
    window.requestAnimationFrame(function () {
      horizonHeader?.style.setProperty('--top-row-height', `${headerTopRow.offsetHeight}px`);
    });
  } else if (customShell instanceof HTMLElement) {
    window.requestAnimationFrame(function () {
